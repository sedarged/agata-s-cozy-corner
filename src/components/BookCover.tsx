      ...
      return (
        <>
          ...
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="absolute rounded-full bg-[#f7f2e8] border border-[#e2d5bf]/70" style={{ width: 18 + (i % 3) * 7, height: 18 + (i % 3) * 7, left: `${8 + i * 10}%`, top: `${8 + (i % 2) * 12}%` }} />
          ))}
      ...
      return (
        <>
          ...
          {Array.from({ length: 10 }).map((_, i) => (
            <div key={i} className="absolute rounded-full bg-[#fff7e8] border border-[#d8c7a7]/60" style={{ width: 20 + (i % 4) * 6, height: 20 + (i % 4) * 6, left: `${4 + i * 9}%`, bottom: `${-1 + (i % 3) * 7}%` }} />
          ))}
      ...
      return (
        <>
          ...
          {Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className="absolute rounded-full bg-[#d7b36d]" style={{ width: i % 3 === 0 ? 3 : 2, height: i % 3 === 0 ? 3 : 2, left: `${10 + i * 9}%`, top: `${8 + (i % 4) * 8}%` }} />
          ))}
      ...
      return (
        <>
          ...
          {Array.from({ length: 7 }).map((_, i) => (
            <div key={i} className="absolute w-[2px] rounded-full bg-[#5a4a3c]/70" style={{ height: `${18 + (i % 3) * 10}%`, right: `${6 + i * 7}%`, bottom: `${15 + (i % 2) * 2}%`, transform: `rotate(${8 - i * 3}deg)` }} />
          ))}
        </>
      );
    case "Siedem sióstr":
      return (
        <>
          ...
          {Array.from({ length: 10 }).map((_, i) => (
            <div key={i} className="absolute w-[5%] h-[22%] bg-[#7d547b] rounded-t-full" style={{ left: `${2 + i * 9}%`, bottom: 0 }} />
          ))}
        </>
      );
    case "Cztery wiatry":
      return (
        <>
          ...
          {Array.from({ length: 11 }).map((_, i) => (
            <div key={i} className="absolute rounded-full border border-[#b99b64]/55" style={{ width: 16 + (i % 3) * 8, height: 16 + (i % 3) * 8, left: `${3 + (i % 4) * 21}%`, top: `${6 + Math.floor(i / 4) * 21}%` }} />
          ))}
    ...
    case "Dom przy ulicy Amélie":
      return (
        <>
          ...
          {Array.from({ length: 7 }).map((_, i) => (
            <div key={i} className="absolute rounded-full bg-[#b17982]/55" style={{ width: 7, height: 7, left: `${18 + i * 9}%`, top: `${10 + (i % 3) * 6}%` }} />
          ))}
        </>
      );
    case "Nocny ogród":
...
function IllustratedFallback({ book, size, className }: { book: BookLike; size: Props["size"]; className?: string }) {
  ...
  return (
    <div className={cn("relative isolate shrink-0 rounded-[4px] book-shadow overflow-hidden", sizes[size ?? "md"], className)}>
      ...
      <div className="relative z-10 h-full p-[10%] flex flex-col justify-between">
        <div className={cn("font-serif leading-[1.02] tracking-[0.06em] text-center", titleClass)} style={{ color: ["Północna biblioteka", "Cztery wiatry", "Nocny ogród", "Atlas szepczących chmur"].includes(book.title) ? "#f2ddb2" : "#3d2b1f" }}>
          {book.title}
        </div>
        {book.author && (
          <div className="text-center uppercase tracking-[0.16em] opacity-85" style={{ fontSize: "0.48em", color: ["Północna biblioteka", "Cztery wiatry", "Nocny ogród", "Atlas szepczących chmur"].includes(book.title) ? "#e6c894" : "#705645" }}>
            {book.author}
          </div>
        )}
      </div>
