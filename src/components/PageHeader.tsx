    ...
    <div className="flex gap-2 overflow-x-auto px-5 lg:px-10 pb-2 -mx-1 no-scrollbar">
      {items.map((i) => (
        <button
          key={i}
          onClick={() => onChange(i)}
          className={`px-4 py-1.5 rounded-full text-sm whitespace-nowrap transition border ${value === i ? "bg-primary text-primary-foreground border-primary" : "bg-card text-foreground border-border hover:bg-muted"}`}
        >
          {i}
        </button>
      ))}
