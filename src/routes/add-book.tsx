...
import { ArrowLeft, Search, Hash, ScanLine, PenLine, Loader2, X, Upload, Camera, CameraOff } from "lucide-react";
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 pb-16 max-w-3xl">
      <div className="flex items-center gap-3 pt-4">
        <Link to="/library" aria-label="Wróć" className="w-10 h-10 grid place-items-center rounded-full glass text-warm hover:bg-[var(--glass-inner)]">
      ...
      {tab === "isbn" && <IsbnTab />}
      {tab === "scan" && <ScanTab onIsbn={(v) => { setTab("isbn"); window.dispatchEvent(new CustomEvent("agata-prefill-isbn", { detail: v })); }} />}
  ...
  return (
    <div className="space-y-4">
      <div className="glass rounded-2xl p-3 flex gap-2">
        ...
        />
        <button onClick={onSearch} disabled={loading || !q.trim()} className="px-4 py-2.5 rounded-xl bg-[var(--accent-gold)] text-[var(--bg)] text-sm font-medium disabled:opacity-50 inline-flex items-center gap-1.5">
      ...
      {results && results.length > 0 && (
        <div className="space-y-3">
          {results.map(r => <ResultCard key={`${r.source}-${r.external_id}`} r={r} />)}
        </div>
      )}
    </div>
  );
  ...
  if (dup) {
    return (
      <div className="glass rounded-2xl p-4 space-y-3">
        ...
        <div className="text-sm text-warm-muted">{dup.title} — {dup.author}</div>
        <div className="flex gap-2 flex-wrap">
          <Link to="/book/$id" params={{ id: dup.id }} className="px-3 py-1.5 rounded-full bg-[var(--accent-gold)] text-[var(--bg)] text-xs">Otwórz książkę</Link>
          <button onClick={() => add(true)} className="px-3 py-1.5 rounded-full glass text-warm text-xs">Dodaj mimo to</button>
          <button onClick={() => setDup(null)} className="px-3 py-1.5 rounded-full glass text-warm text-xs">Anuluj</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="glass rounded-2xl p-3 flex gap-3">
      <div className="w-[64px] h-[96px] rounded-lg overflow-hidden bg-[var(--glass-inner)] shrink-0 grid place-items-center">
        {r.cover_url ? <img src={r.cover_url} alt="" className="w-full h-full object-cover" loading="lazy" /> : <span className="text-[10px] text-warm-muted">Brak okładki</span>}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[11px] text-warm-muted mt-1">
          {[r.published_date, r.page_count ? `${r.page_count} s.` : null, r.category].filter(Boolean).join(" · ") || "Brak danych"}
        </div>
        <button onClick={() => add(false)} className="mt-2 px-3 py-1.5 rounded-full bg-[var(--accent-gold)] text-[var(--bg)] text-xs font-medium">Dodaj do biblioteki</button>
...
function IsbnTab() {
  ...
  const lookup = async () => {
    setError(null); setResult(null); setDup(null);
    if (!valid) { setError("Nieprawidłowy numer ISBN"); return; }
  ...
  const add = (force = false) => {
    if (!result) return;
    const existing = !force && isDuplicateBook({ isbn: result.isbn || clean, title: result.title, author: result.author });
    if (existing) { setDup(existing); return; }
    ...
    <div className="space-y-4">
      <div className="glass rounded-2xl p-3 flex gap-2">
        <input
          ...
          className="flex-1 bg-[var(--glass-inner)] rounded-xl px-4 py-2.5 text-sm text-warm focus:outline-none"
        />
        <button onClick={lookup} disabled={loading} className="px-4 py-2.5 rounded-xl bg-[var(--accent-gold)] text-[var(--bg)] text-sm font-medium disabled:opacity-50 inline-flex items-center gap-1.5">
      ...
      {dup && (
        <div className="glass rounded-2xl p-4 space-y-3">
          ...
          <div className="flex gap-2 flex-wrap">
            <Link to="/book/$id" params={{ id: dup.id }} className="px-3 py-1.5 rounded-full bg-[var(--accent-gold)] text-[var(--bg)] text-xs">Otwórz książkę</Link>
            {result && <button onClick={() => add(true)} className="px-3 py-1.5 rounded-full glass text-warm text-xs">Dodaj mimo to</button>}
          </div>
        </div>
      )}
      {result && !dup && (
        <div className="glass rounded-2xl p-4 flex gap-3">
          <div className="w-[80px] h-[120px] rounded-lg overflow-hidden bg-[var(--glass-inner)] shrink-0 grid place-items-center">
            {result.cover_url ? <img src={result.cover_url} alt="" className="w-full h-full object-cover" /> : <span className="text-[10px] text-warm-muted">Brak okładki</span>}
          </div>
          <div className="flex-1 min-w-0">
            <div className="font-serif text-warm">{result.title}</div>
            <div className="text-xs text-warm-muted">{result.author}</div>
            <div className="text-[11px] text-warm-muted mt-1">{[result.published_date, result.page_count ? `${result.page_count} s.` : null].filter(Boolean).join(" · ") || "Brak danych"}</div>
            <button onClick={() => add(false)} className="mt-2 px-3 py-1.5 rounded-full bg-[var(--accent-gold)] text-[var(--bg)] text-xs font-medium">Dodaj do biblioteki</button>
          </div>
        </div>
      )}
    ...
    <div className="space-y-3">
      <div className="glass rounded-2xl p-3">
        ...
        <div className="flex gap-2 mt-3 flex-wrap">
          {!running ? (
            <button onClick={start} className="px-4 py-2 rounded-full bg-[var(--accent-gold)] text-[var(--bg)] text-sm inline-flex items-center gap-1.5">
              <Camera className="w-4 h-4" /> Uruchom kamerę
            </button>
          ) : (
            <button onClick={stop} className="px-4 py-2 rounded-full glass text-warm text-sm inline-flex items-center gap-1.5">
      ...
      {found && (
        <div className="glass rounded-2xl p-4 space-y-3">
          <div className="text-warm">Znaleziono ISBN: <span className="font-mono">{found}</span></div>
          <button
...
function ManualTab() {
  ...
  return (
    <div className="space-y-3">
      <Field label="Tytuł"><input value={title} onChange={e => setTitle(e.target.value)} className={input} /></Field>
      <Field label="Autor"><input value={author} onChange={e => setAuthor(e.target.value)} className={input} /></Field>
      <Field label="ISBN"><input value={isbn} onChange={e => setIsbn(e.target.value)} className={input} /></Field>
      <Field label="Opis"><textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} className={input} /></Field>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Liczba stron"><input value={pageCount} onChange={e => setPageCount(e.target.value)} inputMode="numeric" className={input} /></Field>
        <Field label="Data wydania"><input value={publishedDate} onChange={e => setPublishedDate(e.target.value)} className={input} /></Field>
      </div>
      <Field label="Gatunek"><input value={genre} onChange={e => setGenre(e.target.value)} className={input} /></Field>
      <Field label="Wydawnictwo"><input value={publisher} onChange={e => setPublisher(e.target.value)} className={input} /></Field>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Seria"><input value={seriesName} onChange={e => setSeriesName(e.target.value)} className={input} /></Field>
        <Field label="Część serii"><input value={seriesPart} onChange={e => setSeriesPart(e.target.value)} className={input} /></Field>
      </div>
      <Field label="Tagi"><input value={tags} onChange={e => setTags(e.target.value)} placeholder="oddziel przecinkiem" className={input} /></Field>
      <Field label="Okładka">
        <div className="space-y-2">
          <input value={coverUrl} onChange={e => setCoverUrl(e.target.value)} placeholder="URL okładki" className={input} />
          <label className="inline-flex items-center gap-2 px-3 py-2 rounded-xl glass text-warm text-sm cursor-pointer w-fit">
            ...
            <input type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) onCover(f); }} />
          </label>
          {(coverData || coverUrl) && (
            <div className="relative w-[80px] h-[120px] rounded-lg overflow-hidden bg-[var(--glass-inner)]">
              ...
              {coverData && (
                <button type="button" onClick={() => setCoverData(null)} className="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/60 text-white grid place-items-center">
                  <X className="w-3 h-3" />
                </button>
              )}
            </div>
      ...
      {dup && (
        <div className="glass rounded-2xl p-4 space-y-3">
          ...
          <div className="flex gap-2 flex-wrap">
            <Link to="/book/$id" params={{ id: dup.id }} className="px-3 py-1.5 rounded-full bg-[var(--accent-gold)] text-[var(--bg)] text-xs">Otwórz książkę</Link>
            <button onClick={() => submit(true)} className="px-3 py-1.5 rounded-full glass text-warm text-xs">Dodaj mimo to</button>
            <button onClick={() => setDup(null)} className="px-3 py-1.5 rounded-full glass text-warm text-xs">Anuluj</button>
          </div>
        </div>
      )}
      ...
      <div className="pt-2">
        <button onClick={() => submit(false)} disabled={busy} className="w-full py-3 rounded-full bg-[var(--accent-gold)] text-[var(--bg)] font-medium disabled:opacity-50">
...
const input = "w-full bg-[var(--glass-inner)] rounded-xl px-3.5 py-2.5 text-sm text-warm focus:outline-none";
