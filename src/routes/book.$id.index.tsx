  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 pb-16">
      <div className="flex items-center justify-between pt-2 pb-3">
        ...
        <span className="text-[11px] uppercase tracking-[0.24em] text-warm-muted">
          Szczegóły książki
        </span>
        <button
          onClick={toggleFav}
        ...
        >
          <Heart
            className={`w-4 h-4 ${fav ? "fill-[var(--accent-gold)] text-[var(--accent-gold)]" : "gold-text"}`}
          />
        </button>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 inline-flex items-center justify-center gap-2 py-2.5 rounded-full glass text-warm text-sm"
            >
    ...
    <div className="fixed inset-0 z-50 bg-black/40 grid place-items-end sm:place-items-center p-2 sm:p-6 overflow-y-auto">
      <div className="glass rounded-3xl w-full max-w-lg p-5 space-y-3 max-h-[92vh] overflow-y-auto">
        ...
        <label className="block">
          <span className="text-[11px] uppercase tracking-wider text-warm-muted">Tytuł</span>
          <input value={title} onChange={(e) => setTitle(e.target.value)} className={inp} />
        </label>
        <label className="block">
          <span className="text-[11px] uppercase tracking-wider text-warm-muted">Autor</span>
          <input value={author} onChange={(e) => setAuthor(e.target.value)} className={inp} />
        </label>
        <label className="block">
          <span className="text-[11px] uppercase tracking-wider text-warm-muted">ISBN</span>
          <input value={isbn} onChange={(e) => setIsbn(e.target.value)} className={inp} />
        </label>
        <label className="block">
          <span className="text-[11px] uppercase tracking-wider text-warm-muted">Opis</span>
          <textarea
            value={description}
        ...
        <label className="block">
          <span className="text-[11px] uppercase tracking-wider text-warm-muted">Gatunek</span>
          <input value={genre} onChange={(e) => setGenre(e.target.value)} className={inp} />
        </label>
        <label className="block">
          <span className="text-[11px] uppercase tracking-wider text-warm-muted">Wydawnictwo</span>
          <input value={publisher} onChange={(e) => setPublisher(e.target.value)} className={inp} />
        </label>
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="text-[11px] uppercase tracking-wider text-warm-muted">Seria</span>
            <input
              value={seriesName}
        ...
        <label className="block">
          <span className="text-[11px] uppercase tracking-wider text-warm-muted">Tagi</span>
        ...
        <div className="flex gap-2 pt-2">
          <button
            onClick={save}
            className="flex-1 py-2.5 rounded-full bg-[var(--accent-gold)] text-[var(--bg)] text-sm font-medium"
          >
            Zapisz
          </button>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center p-4">
      <div className="glass rounded-3xl w-full max-w-sm p-5 space-y-3">
        ...
        <div className="flex gap-2 pt-2">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-full glass text-warm text-sm">
            Anuluj
          </button>
          <button
            onClick={onConfirm}
...
function RatingEditor({
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <div className="flex items-center gap-1">
          {[1, 2, 3, 4, 5].map((i) => (
            ...
            >
              <Star
                className={`w-6 h-6 ${i <= stars ? "fill-[var(--accent-gold)] text-[var(--accent-gold)]" : "text-warm-muted"}`}
              />
            </button>
          ))}
        </div>
      ...
      <div>
        <div className="text-[11px] uppercase tracking-wider text-warm-muted mb-1.5">
          Prywatna opinia
        </div>
        <textarea
          value={opinion}
