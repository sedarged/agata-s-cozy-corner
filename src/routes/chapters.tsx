...
export const Route = createFileRoute("/chapters")({
  head: () => ({ meta: [{ title: "Chapter notes — Agata" }] }),
  component: Chapters,
});
...
function Chapters() {
  ...
  return (
    <div>
      <PageHeader title="Chapters" subtitle="Notes organised by chapter."/>
      <div className="px-5 lg:px-10 grid lg:grid-cols-2 gap-4 pb-12 max-w-5xl">
            ...
            <Link to="/note/$id" params={{ id: n.id }} key={n.id} className="flex gap-4 p-5 bg-card rounded-2xl shadow-soft hover:shadow-warm transition">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between gap-3">
                  <div className="font-medium">Chapter {n.chapterNumber}</div>
        ...
        <Link to="/note/new?type=chapter" className="flex items-center justify-center gap-2 p-5 rounded-2xl border-2 border-dashed border-border text-muted-foreground hover:text-primary hover:border-primary"><Plus className="w-4 h-4"/>New chapter note</Link>
      </div>
    </div>
  );
}
