  ...
  return (
    <div className="min-h-screen flex flex-col bg-[oklch(0.95_0.01_60)]">
      <div className="flex items-center justify-between px-5 py-3 bg-card border-b border-border">
        ...
        <button className="flex items-center gap-2 px-4 py-2 rounded-full bg-primary text-primary-foreground text-sm"><Save className="w-4 h-4"/>Zapisz</button>
      </div>
      ...
      <div className="flex-1 grid grid-cols-[60px_1fr] lg:grid-cols-[60px_1fr_300px] gap-4 p-4">
        ...
        <div className="bg-[oklch(0.98_0.015_75)] rounded-2xl shadow-soft p-10 relative overflow-hidden" style={{ backgroundImage: "repeating-linear-gradient(transparent, transparent 31px, oklch(0.92 0.02 60) 32px)" }}>
          <div className="font-serif text-3xl" style={{ color, fontFamily: "Fraunces, serif", fontStyle: "italic" }}>Ten fragment mnie złamał.</div>
          <blockquote className="mt-6 font-serif italic text-lg max-w-md leading-relaxed" style={{ color }}>„Nie wznosisz się na poziom swoich celów. Spadasz do poziomu swoich systemów."</blockquote>
          <div className="mt-4 font-serif text-2xl italic" style={{ color: "#8a485a" }}>Pamiętaj to zawsze.</div>
          <div className="mt-10 text-base italic" style={{ color }}>Rozwój Violet to wszystko.</div>
          <div className="absolute top-10 right-10 w-44 h-44 rounded-2xl bg-gradient-to-br from-accent to-muted grid place-items-center text-muted-foreground text-xs">📷 szkic</div>
          <div className="absolute bottom-6 left-10 text-xs text-muted-foreground italic">— zacznij pisać piórkiem —</div>
        </div>
        ...
        <aside className="hidden lg:flex flex-col gap-4">
          <div className="bg-card rounded-2xl p-5 shadow-soft">
            ...
            <div className="mt-3 space-y-2">
              {["Streszcz to","Wyjaśnij ten cytat","Zaproponuj tag","Pokaż bibliotekę komentarzy"].map(t => (
