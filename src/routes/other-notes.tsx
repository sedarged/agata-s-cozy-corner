...
function Other() {
  ...
  return (
    <div>
      <PageHeader title="Inne notatki" subtitle="Teorie, listy postaci, przypomnienia, luźne myśli."/>
          ...
          return (
            <Link to="/note/$id" params={{ id: n.id }} key={n.id} className="flex items-center gap-4 p-4 bg-card rounded-2xl shadow-soft hover:shadow-warm transition">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] text-muted-foreground mt-1">{book.title} · Zaktualizowano {n.createdAt}</div>
              </div>
              <ChevronRight className="w-4 h-4 text-muted-foreground"/>
            </Link>
          );
        })}
        <Link to="/note/new?type=other" className="flex items-center justify-center gap-2 p-4 rounded-2xl border-2 border-dashed border-border text-muted-foreground hover:text-primary hover:border-primary"><Plus className="w-4 h-4"/>Nowa notatka</Link>
      </div>
    </div>
  );
}
