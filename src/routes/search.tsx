...
function SearchPage() {
  ...
  const [tab, setTab] = useState("Wszystko");
    ...
    <div>
      <div className="px-5 lg:px-10 pt-8 flex items-center gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input value={q} onChange={e => setQ(e.target.value)} className="w-full bg-card border border-border rounded-full pl-11 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="Szukaj po tytule, autorze lub ISBN"/>
        </div>
      </div>
      <PageHeader title={<span className="text-xl lg:text-2xl">Wyniki dla „{q}"</span>} />
      <Chips items={["Wszystko","Tytuły","Autorzy","ISBN"]} value={tab} onChange={setTab}/>
        ...
        {results.map(b => (
          <div key={b.id} className="flex gap-4 p-4 rounded-2xl bg-card shadow-soft">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-muted-foreground mt-1">{b.publishedDate} · {b.genre} · {b.pageCount} stron</div>
              <div className="flex gap-2 mt-3">
                <button className="px-3 py-1.5 rounded-full bg-primary text-primary-foreground text-xs">Dodaj</button>
